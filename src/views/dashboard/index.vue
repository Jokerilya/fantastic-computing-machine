...
export default {
  ...
  data() {
	...
	methods:{
		...
		setOptions() {
			let xAxisData = [],seriesData = [],text = '周'
		    ...
		    this.chart.setOption({
				title: {
					left:'150px',
					text: '本'+text+'订单总数 ' + this.chartData.num || 0 + '     本'+text+'订单总额 ' + this.chartData.money || 0,
				},
				tooltip: {},
				xAxis: {
					data:xAxisData
...
<style lang="scss">
.count-wrap{background: #fff;padding: 18px;}
.count-title{padding: 18px 0;font-weight: bold;font-size: 17px;}
.count-top{display: flex;flex-direction: column;justify-content: space-around;padding:10px 0 10px 34%;position: relative;color: #fff;
	>i{font-size: 52px;position: absolute;left: 18%;top: 50%;transform: translate(-46%,-50%);}
	>span{font-size: 14px;margin: 4px 0;}
	>span:nth-of-type(2){font-size: 20px;font-weight: bold;}
}
.count-content{display: flex;flex-direction: column;padding:16px 0 16px 0;position: relative;
	>i{font-size: 32px;background: #f4c735;border-radius: 50%;position: absolute;left: 18%;top: 50%;transform: translate(-46%,-50%);padding: 14px;color: #fff;}
	.count-content-box{
		padding-left: 30%;display: flex;flex-direction: column;justify-content: space-around;align-items: center;width: 252px;
		span{font-size: 16px;margin: 4px 0;font-weight: bold;}
		span:nth-of-type(2){font-size: 14px;color: #454545;}	
	}
}
.count-content::before{content: '';width: 1px;height: 54px;background: #eee;position: absolute;right: 0;top: 50%;transform: translateY(-50%);}
...
.myCharts{height: 350px;}
.count-tab{
	display: flex;align-items: center;justify-content: flex-end;padding-right: 200px;
	span{margin-left: 12px;font-weight: 600;font-size: 14px;cursor: pointer;}
	span.cur{color: #409EFF;}
}
</style>
