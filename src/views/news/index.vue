	...
	<div class="app-container">
		<div class="manage-top" style="display: flex;">
			<el-form label-width="88px" label-position="right" style="width: 100%;">
				<el-row :gutter="20">
					<el-col>
						<el-button style="margin-bottom: 22px;" type="primary" icon="el-icon-plus" @click="addNews('new')">添加资讯</el-button>
					</el-col>
				</el-row>
			</el-form>
		</div>
		...
		<div class="manage-table">
			<el-table ref="multipleTable" :data="dataList" v-loading="dataConfig.loading" style="width: 100%" border :header-cell-style="{color:'#666'}">
				...
				<el-table-column label="封面图片" width="300" align="center">
					<template slot-scope="scope">
						<el-image style="height: 80px;" :preview-src-list="[scope.row.coverPicture]" :src="scope.row.coverPicture"></el-image>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" width="150" align="center" fixed="right">
					<template slot-scope="scope">
						<el-button type="text" @click="addNews('edit',scope.row)">编辑</el-button>
		...
		<el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="1000px">
			<el-form :model="newsForm" :rules="rules" ref="newsForm" label-width="110px" style="padding-right: 20px;">
				<el-form-item label="资讯标题" prop="title">
					<el-input v-model="newsForm.title" placeholder="请输入新闻资讯的标题"></el-input>
				</el-form-item>
				<el-form-item label="资讯封面图" prop="coverPicture">
					<upload-img width="80px" height="80px" :img="newsForm.coverPicture" @uploadSuc="uploadSucFn"/>
				</el-form-item>
				<el-form-item label="资讯详情" prop="content">
					<tinymce v-if="dialogVisible" v-model="newsForm.content" :height="600"/>
				</el-form-item>
			</el-form>
			<span slot="footer" style="padding-right: 20px;">
				<el-button type="primary" @click="addNewsSubmit('newsForm')">确定{{dialogTitle === '编辑新闻资讯'?'编辑':'添加'}}</el-button>
			</span>
		</el-dialog>
	</div>
</template>
...
<script>
	import {queryOperationNews,getOperationNewsDetail,editOperationNews,delOperationNews} from '@/api/news.js'
