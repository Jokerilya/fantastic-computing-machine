<template>
  <div class="content_box">
    ...
    <div class="select_view">
      ...
      <el-input v-model="queryData.orderSn" style="width: 200px" class="mg_r15 mg_t10" placeholder="请输入订单号" />
      下单时间：
      <el-date-picker v-model="queryData.queryTime" class="mg_r15 mg_t10" type="daterange" value-format="yyyy-MM-dd"
        range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" />
      <el-select v-model="queryData.status" class="mg_r15 mg_t10" style="width: 160px" placeholder="请选择订单状态">
        <el-option v-for="item in orderStatus" :key="item.val" :label="item.name" :value="item.val" />
      </el-select>
      ...
      <el-button class="mg_t10" type="primary" @click="exportExlFn">订单导出</el-button>
    </div>
    <el-table v-loading="dataConfig.loading" border :data="dataList" class="list">
      ...
      <el-table-column label="推荐用户昵称" align="center" header-align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.parentName || "暂无数据" }}</span>
        </template>
      </el-table-column>
      <el-table-column label="推荐用户手机号" align="center" header-align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" fixed="right" width="250" align="center" header-align="center">
        <template slot-scope="scope">
          <el-button type="primary" plain size="small" @click="getOrderDetailFn(scope.row.id)">详情</el-button>
          <el-button type="primary" :disabled="scope.row.status != 5" size="small" @click="orderSend(scope.row.id)">发货
          </el-button>
          <!-- <el-button
            type="primary"
    ...
    <el-dialog title="订单详情" :visible.sync="orderDetailVisible" width="1000px">
      <div class="order_detail">
        <div v-for="(item, key) in orderDetailTitle" :key="key" class="detail" :class="{ detail2: key == 'list' }">
          <span class="tit">{{ orderDetailTitle[key] }}</span>
          <span v-if="key == 'status'">{{
            orderStatusText["statu_" + orderDetail[key]]
          }}</span>
          <el-table v-else-if="key == 'list'" border :data="orderDetail[key]" style="width: 82%">
            <el-table-column label="商品名称">
              <template slot-scope="scope">
                <el-tooltip class="item" effect="dark" :content="scope.row.productName" placement="top-start">
                  <span class="pro_name">{{ scope.row.productName }}</span>
                </el-tooltip>
              </template>
            </el-table-column>
            <el-table-column prop="price" align="center" header-align="center" label="商品单价" />
            <el-table-column label="商品规格">
              <template slot-scope="scope">
                <el-tooltip class="item" effect="dark" :content="scope.row.skuName" placement="top-start">
            ...
            <el-table-column prop="quantity" align="center" header-align="center" label="购买数量" />
            <!-- <el-table-column prop="weight" align="center" header-align="center" label="购买重量" /> -->
          </el-table>
          <span v-else-if="key == 'expNo'">
            <el-link v-if="orderDetail[key]" target="_block" :underline="false"
              :href="`https://m.kuaidi100.com/result.jsp?nu=${orderDetail[key]}`" type="primary">{{ orderDetail[key] }}
            </el-link>
    ...
    <el-dialog title="订单发货" :visible.sync="orderSendVisible" width="600px">
      <el-form ref="form" :model="orderSendData" label-width="80px">
        ...
        <el-form-item label="物流公司">
          <el-select filterable v-model="orderSendData.expId" style="width: 100%" placeholder="请选择物流公司">
            <el-option v-for="item in expList" :key="item.id" :label="item.expName" :value="item.id" />
          </el-select>
        </el-form-item>
    ...
    methods: {
      ...
      exportExlFn() {
        handleOrderExportFn({
          ...
          queryTime: this.queryData.queryTime.length == 0 ?
            "" :
