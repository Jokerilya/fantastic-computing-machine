...
<template>
  <div class="app-container">
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="7" style="text-align: right">
        ...
        <el-button plain type="primary" @click="query_queryRepairOrderList"
          >查询</el-button
        >
        <el-button plain type="info" @click="resetFn" style="margin-right: 10px"
          >重置</el-button
        >
        <el-dropdown
        ...
        <el-dropdown @command="tableListFn" trigger="click">
          <el-button type="success" plain> 表格 </el-button>
          <el-dropdown-menu slot="dropdown">
      ...
      >
        ...
        >
          <template slot-scope="{ row }">
            <el-button type="text" @click="queryDesc(row)">{{
              row.orderSn
            }}</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog
      title="订单总表"
      fullscreen
      :visible="orderSummaryTableVisible"
      :before-close="closeOrderSummaryTableDialog"
    >
      ...
      <div class="orderSummaryTableFoot">
        <div class="orderSummaryTablePagination">
          <el-pagination
            v-if="!orderSummaryTableListEdit"
            background
            layout="prev, pager, next"
            :total="orderSummaryTableListTotal"
            @current-change="changeOrderSummaryTablePage"
          >
          </el-pagination>
        </div>
        <div class="dialog-footer">
          <el-button
            ...
            >编 辑</el-button
          >
          <el-button type="success" v-else @click="supplementOrderInfo"
            >保 存</el-button
          >
          <!-- <el-button @click="closeOrderSummaryTableDialog">取 消</el-button> -->
          <el-button
            type="primary"
            @click="handleOnlineRepairEnterpriseOrderExport"
            v-if="!orderSummaryTableListEdit"
            >导 出</el-button
          >
        </div>
    ...
    <!-- 交接表  -->
    <el-dialog
      :before-close="closeChecklistTableDialog"
      :visible="checklistTableVisible"
      title="订单总表"
      fullscreen
    >
      <el-table border :data="checklistTableList">
        ...
        <el-table-column label="工单渠道" width="120" align="center">
          <template slot-scope="{ row }"> 线上渠道 </template>
        </el-table-column>
        <el-table-column
          prop="masterRealName"
          label="师傅"
        ...
        <el-table-column label="师傅人工" width="110" align="center">
          <template slot-scope="{ row }"> 0 </template>
        </el-table-column>
        ...
        <el-table-column label="客户验收" width="120" align="center">
          <template slot-scope="{ row }"> 12月11日 </template>
        </el-table-column>

        <el-table-column label="工单属性" width="160" align="center">
          <template slot-scope="{ row }">
            <el-select placeholder="选择工单属性">
              <el-option :key="1" label="维修类" :value="1"> </el-option>
              <el-option :key="2" label="保养类" :value="2"> </el-option>
            </el-select>
          </template>
        </el-table-column>
        ...
        <el-table-column label="师傅代买配件费" width="170" align="center">
          <template slot-scope="{ row }">
            <el-input size="small" placeholder="输入师傅代买配件费"></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column label="提成" width="150" align="center">
          <template slot-scope="{ row }">
            <el-input size="small" placeholder="输入提成"></el-input>
          </template>
        </el-table-column>
        <el-table-column label="单量" width="150" align="center">
          <template slot-scope="{ row }">
            <el-input size="small" placeholder="输入单量"></el-input>
          </template>
        </el-table-column>
        <el-table-column label="备注" width="230" align="center">
          <template slot-scope="{ row }">
            <el-input size="small" placeholder="输入备注"></el-input>
          </template>
        </el-table-column>
      </el-table>
      <div class="orderSummaryTableFoot">
        ...
        <div class="dialog-footer">
          <el-button type="warning">编 辑</el-button>
...
export default {
  ...
  methods: {
    // 关闭交接单
    closeChecklistTableDialog() {
      this.checklistTableVisible = false;
    },
    // 单量不能小于0
    changeValidOrderNum(row) {
      let val = row.validOrderNum;
    ...
    // 补充订单主表信息
    async supplementOrderInfo() {
      const res = await supplementOrderInfo(this.orderSummaryTableList);
      if (res.code == "000") {
        this._queryRepairOrderList();
    ...
    // 总表
    async handleOnlineRepairEnterpriseOrderExport() {
      if (this.orderSummaryTableListTotal > 1000) {
        this.$message({
      ...
      let prams = JSON.parse(JSON.stringify(this.orderSummaryTableListParams));
      prams.pageSize = 9999;
      const res = await handleOnlineRepairEnterpriseOrderExport(prams);
      if (res) {
    ...
    // 切换总表列表页码
    async changeOrderSummaryTablePage(page) {
      this.orderSummaryTableListParams.pageNo = page;
      const res = await queryRepairOrderList(this.orderSummaryTableListParams);
      if (res.code == "000") {
        this.orderSummaryTableList = res.data.records;
        this.orderSummaryTableListTotal = res.data.total;
      }
    },
    // 查询总表列表
    async queryOrderSummaryTableList() {
    ...
    async tableListFn(e) {
      ...
      // 打开总表
      if (e == "订单总表") {
        await this.queryOrderSummaryTableList();
        this.orderSummaryTableVisible = true;
      } else if (e == "交接单") {
        this.checklistTableVisible = true;
      }
    },
    // 点击导出按钮
    exportListFn(e) {
